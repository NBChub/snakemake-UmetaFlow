...
# 2) Correct the MS2 precursor in a feature level.        
...
rule precursorcorrection_feature:
    ...
    shell:
        """
        resources/OpenMS-2.7.0/bin/HighResPrecursorMassCorrector -in {input[0]} -feature_in {input[1]} -out {output}  -nearest_peak:mz_tolerance "100.0"
        """ 

# 3) SIRIUS generates formula prediction from scores calculated from 1) MS2 fragmentation scores (ppm error + intensity) and 2) MS1 isotopic pattern scores.        
...
# 4) Convert the mzTab to a csv file
